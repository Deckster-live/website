...
export function Navbar() {
  ...
  return (
    ...
    >
      <div className="mx-auto grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 py-3.5 md:px-8 lg:grid-cols-[1fr_auto_1fr]">
        {/* Logo */}
        <Link
          href="#top"
        ...
        >
          {links.map(({ label, href }) => (
            <Link
              key={href}
              href={href}
              className="text-base text-slate font-medium transition-colors hover:text-foreground"
            >
              {label}
            </Link>
          ))}
      ...
      {open && (
        <div className="border-t border-line bg-background/95 backdrop-blur-xl lg:hidden">
          <nav
            className="mx-auto flex max-w-310 flex-col px-5 py-2 md:px-8"
